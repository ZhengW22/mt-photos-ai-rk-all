...
| 变量                      | 默认值                   | 说明                                                                                     |
| ------------------------- | ------------------------ | ---------------------------------------------------------------------------------------- |
| `RKNN_TARGET`           | `rk3588`               | 目标芯片型号，支持 `rk3588`（3核NPU）和 `rk3568`（1核NPU）                            |
| `API_AUTH_KEY`          | `mt_photos_ai_extra`   | 所有 API 访问时需要在 Header 中传入 `api-key`                                          |
| `SERVER_RESTART_TIME`   | `300`                  | (秒) 当容器长时间没有请求时自动重启整个进程，设为 `0` 可关闭                           |
| `FACE_RECOGNITION_MODEL`| 随 `RKNN_TARGET` 自动选 | 人脸识别模型名称，rk3588 默认 `Gundam_RK3588`，rk3568 默认 `Gundam_RK356X`           |
